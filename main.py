...
# Fetch live data
gold_price_oz_usd, gold_time = get_gold_price()
usd_to_thb, fx_time = get_usd_to_thb()
...
if usd_to_thb is not None and fx_time is not None:
    # Converting to UCT+7
...
if gold_price_oz_usd is not None and usd_to_thb is not None:
...
oz_total = gold_price_per_oz_thb * oz_qty if gold_price_oz_usd is not None else 0
...
gram_total = gold_price_per_gram_thb * gram_qty if gold_price_oz_usd is not None else 0
...
baht_total = gold_price_1baht_thb * baht_qty if gold_price_oz_usd is not None else 0
